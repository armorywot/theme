...
add_filter('pre_get_posts', 'custom_order_posts');

function custom_order_posts($query) {
...
add_action('admin_menu', 'add_admin_wargag_options');

function add_admin_wargag_options() {
    add_options_page('Wargag Options', 'Wargag Options', 8, 'wargag-options', 'init_wargag_options');
}

function init_wargag_options() {
    echo '<h2>Wargag Options</h2>';

    setup_wargag_options();
}

function setup_wargag_options(){
    echo '<td><label for="options" class="required">Получить контент из (количество страниц)</label></td>';
    echo '<td><input class="span5" maxlength="30" name="options[page]" id="page" type="text"></td>';
    echo '<br><br>';
    echo '<input type="submit" class="btn btn-default" name="options[save]" value="Save">';
}
...
if (!include_once (get_template_directory() . "/inc/library.php"))
    wp_die("Cannot include file /inc/library.php.");
...
function smt_wp_title($title, $sep) {
    ...
    return "<title>" . sprintf($format, $title) . "</title>\r\n";
}
    ...
    if (isset($i['maxchar'])) {
        ...
        if ($p->post_excerpt) {
            ...
            if (preg_match('/<!--more(.*?)?-->/', $content, $matches)) {
                ...
                if ($echo)
                    return print force_balance_tags($content[0]);
                else
        ...
        if ($menu_items = wp_get_nav_menu_items($menu->term_id)) {
            ...
            foreach ($menu_items as $key => $menu_item) {
                if ($menu_item->menu_item_parent == 0)
                    ...
                    $sub[$menu_item->ID] = $sub[$menu_item->menu_item_parent] . '- ';
